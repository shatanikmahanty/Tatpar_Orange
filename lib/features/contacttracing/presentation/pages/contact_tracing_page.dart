...
class ContactTracingPage extends StatelessWidget {
  ...
  FormGroup _contactTracingFormBuilder(
      {required ContactTracingModel? contactTracingModel, required SourceCubit cubit}) {
    ...
    return fb.group({
      ...
      'tpt_eligible': FormControl<String?>(value: contactTracingModel?.tptEligible),
      'tpt_regimen': FormControl<String?>(value: tptRegimenName ?? contactTracingModel?.tptRegimen),
      'tpt_start_date': FormControl<DateTime>(value: contactTracingModel?.tptStartDate),
      'weight': FormControl<int>(value: contactTracingModel?.weight),
      'tpt_side_effects': FormControl<String?>(value: contactTracingModel?.tptSideEffects),
      'tpt_outcome_date': FormControl<DateTime>(value: contactTracingModel?.tptOutcomeDate),
      'tpt_outcome': FormControl<String>(value: contactTracingModel?.tptOutcome),
    });
  }
  ...
  Future<void> _onSave(BuildContext context, FormGroup formGroup) async {
    if (formGroup.valid) {
      ...
      context.read<CaseCubit>().selectTPTRegimen = sourceCubit.state.diagnosisData?.tptRegimen!
          .firstWhere((element) => element.name == formGroup.control('tpt_regimen').value,
              orElse: () => const TPTRegimen(id: null))
          .id;
      final model = cubit.state.contactTracingModel ?? const ContactTracingModel();
      ...
      await cubit.updateContactTracingData(contactTracingModel, contactTracingCubit.state.contactTracingModel.id);
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ContactTracingCubit, ContactTracingState>(builder: (context, state) {
      return Scaffold(
          appBar: CaseAppBar(
            'Contact Tracing',
            onClick: () {
              context.router
                  .pushAndPopUntil(const ContactTracingListRoute(), predicate: (Route<dynamic> route) => false);
            },
          ),
                            ...
                            child: Padding(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  PrimaryTextField(
                                    ...
                                    label: AppLocalizations.of(context)!.tbContactName,
                                  ...
                                  TextFieldWithList(
                                    ...
                                    onSelected: (value) {
                                      formGroup.control('screening_outcome').value = value[0];
                                    },
                                    emptyString: '',
                                  ),
                                  const SizedBox(height: kPadding * 2),
                                  ChipRadioButtons(
                                    label: AppLocalizations.of(context)!.testConducted,
                                    options: const ['Yes', 'No'],
                                    crossAxisCount: 2,
                                    onChanged: (value) {
                                      formGroup.control('test_conducted').value = value;
                                    },
                                    selected: formGroup.control('test_conducted').value,
                                  ),
                                  const SizedBox(height: kPadding * 2),
                                      ...
                                      builder: (context, control, child) => Visibility(
                                          ...
                                          child: Column(children: [
                                            DateTextInput(
                                              ...
                                              label: AppLocalizations.of(context)!.cxr_date,
                                            ),
                                            const SizedBox(height: kPadding * 2),
                                            ChipRadioButtons(
                                              label: AppLocalizations.of(context)!.cxr_result,
                                              options: const ['Normal', 'Abnormal'],
                                              crossAxisCount: 2,
                                              onChanged: (value) {
                                                formGroup.control('cxr_result').value = value;
                                              },
                                              selected: formGroup.control('cxr_result').value,
                                            ),
                                            const SizedBox(height: kPadding * 2),
                                            DateTextInput(
                                              firstDate: DateTime(2002),
                                              controlName: 'tbi_date',
                                              label: AppLocalizations.of(context)!.tbi_date,
                                            ),
                                            const SizedBox(height: kPadding * 2),
                                            ChipRadioButtons(
                                              label: AppLocalizations.of(context)!.tbi_result,
                                              options: const ['Negative', 'Positive'],
                                              crossAxisCount: 2,
                                              onChanged: (value) {
                                                formGroup.control('tbi_result').value = value;
                                              },
                                              selected: formGroup.control('tbi_result').value,
                                            ),
                                            const SizedBox(height: kPadding * 2),
                                            PrimaryTextField(
                                              formControlName: 'nikshay_id',
                                              label: AppLocalizations.of(context)!.nikshay_id,
                                              prefixIcon: Icons.account_circle_outlined,
                                            ),
                                            const SizedBox(height: kPadding * 2),
                                          ]))),
                                  ChipRadioButtons(
                                    label: AppLocalizations.of(context)!.tpt_eligible,
                                    options: const ['Yes', 'No'],
                                    crossAxisCount: 2,
                                    onChanged: (value) {
                                      formGroup.control('tpt_eligible').value = value;
                                    },
                                    selected: formGroup.control('tpt_eligible').value,
                                  ),
                                  const SizedBox(height: kPadding * 2),
                                  BlocBuilder<SourceCubit, SourceState>(
                                      ...
                                      builder: (context, state) {
                                        List<String> tptRegimen = (state.diagnosisData != null)
                                            ? state.diagnosisData!.tptRegimen!.map((e) => '${e.name}').toList()
                                            : [];
                                        ...
                                        return ChipRadioButtons(
                                          label: AppLocalizations.of(context)!.tpt_regimen,
                                          options: tptRegimen,
                                          crossAxisCount: 2,
                                          onChanged: (value) {
                                            formGroup.control('tpt_regimen').value = value;
                                          },
                                          selected: formGroup.control('tpt_regimen').value,
                                        );
                                      }),
                                  const SizedBox(height: kPadding * 2),
                                  DateTextInput(
                                    ...
                                    label: AppLocalizations.of(context)!.tpt_start_date,
                                  ),
                                  const SizedBox(height: kPadding * 2),
                                  PrimaryTextField<int>(
                                  ...
                                  PrimaryTextField(
                                    ...
                                    label: AppLocalizations.of(context)!.tpt_side_effects,
                                    prefixIcon: Icons.account_circle_outlined,
                                  ),
                                  const SizedBox(height: kPadding * 2),
                                  DateTextInput(
                                    ...
                                    label: AppLocalizations.of(context)!.tpt_outcome_date,
                                  ),
                                  const SizedBox(height: kPadding * 2),
                                  TextFieldWithList(
                                    controlName: 'tpt_outcome',
                                    label: AppLocalizations.of(context)!.tpt_outcome,
                                    padding: EdgeInsets.zero,
                                    prefixIcon: Icons.account_circle_outlined,
                                    listData: const [
                                      'TPT Complete',
                                    ...
                                    allowMultiSelection: false,
                                    onSelected: (value) {
                                      formGroup.control('tpt_outcome').value = value[0];
                                    },
                                    emptyString: '',
                                  ),
                                ])))),
